...
File: prompt_bert.py
Description: Trains the baseline BERT model on the
sarcastic data and then train the same model with prompt
engineering.
...
logs_path = log_dir / 'bert_prompt.log'
...
class SarcasmDataset(Dataset):
    def __init__(self, data_list: List[Dict[str, Any]],
        ...
        """
        Creates the prompt template and appends them to the
        comments.
        """
        self.input_ids = []
...
def train_model_with_cv(dataset: Dataset, model: nn.Module,
                        tokenizer: BertTokenizer, num_epochs: int = 2,
...
if __name__ == "__main__":
    # initialize tokenized dataset for both models
    ...
    # initialize BERT models for prompt engineering and baseline
    model_with_prompt = BertForSequenceClassification.from_pretrained(
